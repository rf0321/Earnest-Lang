...
fix_identifier_expression(Block *current_block,Expression *expr){
    ...
    decl=eaf_search_declaration(expr ->u.identifier.name,current_block);
    ...
    fd =eaf_search_function(expr->u.identifier.name);
    if (fd == NULL) {
        eaf_compile_error(expr->line_number,
    ...
    expr->u.identifier.is_function = EVM_TRUE;
...
static void
cast_mismatch_error(int line_number,EVM_BasicType src, EVM_BasicType dest)
{
    char *src_name = eaf_get_basic_type_name(src);
    char *dest_name = eaf_get_basic_type_name(dest);

    eaf_compile_error(line_number,
                      CAST_MISMATCH_ERR,
...
{
    Expression *cast_expr = eaf_alloc_expression(CAST_EXPRESSION);
    ...
    if (cast_type == INT_TO_DOUBLE_CAST) {
        cast_expr->type = eaf_alloc_type_specifier(EVM_DOUBLE_TYPE);
    } else if (cast_type == DOUBLE_TO_INT_CAST) {
        cast_expr->type = eaf_alloc_type_specifier(EVM_INT_TYPE);
    } else if (cast_type == BOOLEAN_TO_STRING_CAST
               || cast_type == INT_TO_STRING_CAST
               || cast_type == DOUBLE_TO_STRING_CAST) {
        cast_expr->type = eaf_alloc_type_specifier(EVM_STRING_TYPE);
    }
...
create_assign_cast(Expression *src, TypeSpecifier *dest)
{
    ...
    if (src->type->derive != NULL
        || dest->derive != NULL) {
        eaf_compile_error(src->line_number, DERIVE_TYPE_CAST_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    if (src->type->basic_type == dest->basic_type) {
        return src;
    }
    if (src->type->basic_type == EVM_INT_TYPE
        && dest->basic_type == EVM_DOUBLE_TYPE) {
        cast_expr = alloc_cast_expression(INT_TO_DOUBLE_CAST, src);
        return cast_expr;
    } else if (src->type->basic_type == EVM_DOUBLE_TYPE
               && dest->basic_type == EVM_INT_TYPE) {
...
fix_assign_expression(Block *current_block, Expression *expr)
{
    ...
    if (expr->u.assign_expression.left->kind != IDENTIFIER_EXPRESSION) {
        eaf_compile_error(expr->u.assign_expression.left->line_number,
...
eval_math_expression_int(Expression *expr, int left, int right)
{
    ...
    expr->type = eaf_alloc_type_specifier(DVM_INT_TYPE);
...
{
    EVM_Char *left_str = expr->u.binary_expression.left->u.string_value;
    EVM_Char *right_str;
    int         len;
    EVM_Char    *new_str;

    right_str = eaf_expression_to_string(expr->u.binary_expression.right);
    if (!right_str) {
        return expr;
    }
    
    len = evm_wcslen(left_str) + evm_wcslen(right_str);
    new_str = MEM_malloc(sizeof(EVM_Char) * (len + 1));
    evm_wcscpy(new_str, left_str);
    evm_wcscat(new_str, right_str);
    MEM_free(left_str);
    MEM_free(right_str);
    expr->kind = STRING_EXPRESSION;
    expr->type = eaf_alloc_type_specifier(EVM_STRING_TYPE);
...
cast_binary_expression(Expression *expr)
{
    ...
    if (eaf_is_int(expr->u.binary_expression.left->type)
        && eaf_is_double(expr->u.binary_expression.right->type)) {
        cast_expression
            = alloc_cast_expression(INT_TO_DOUBLE_CAST, 
                                    expr->u.binary_expression.left);
        expr->u.binary_expression.left = cast_expression;

    } else if (eaf_is_double(expr->u.binary_expression.left->type)
               && eaf_is_int(expr->u.binary_expression.right->type)) {
        cast_expression
            = alloc_cast_expression(INT_TO_DOUBLE_CAST, 
                                    expr->u.binary_expression.right);
        expr->u.binary_expression.right = cast_expression;

    } else if (eaf_is_string(expr->u.binary_expression.left->type)
               && eaf_is_boolean(expr->u.binary_expression.right->type)) {
        cast_expression
...
fix_math_binary_expression(Block *current_block, Expression *expr)
{
    ...
    if (eaf_is_int(expr->u.binary_expression.left->type)
        && eaf_is_int(expr->u.binary_expression.right->type)) {
        expr->type =eaf_alloc_type_specifier(EVM_INT_TYPE);
    } else if (eaf_is_double(expr->u.binary_expression.left->type)
               && eaf_is_double(expr->u.binary_expression.right->type)) {
        expr->type = eaf_alloc_type_specifier(EVM_DOUBLE_TYPE);
    } else if (eaf_is_string(expr->u.binary_expression.left->type)
               && eaf_is_string(expr->u.binary_expression.right->type)) {
        expr->type = eaf_alloc_type_specifier(EVM_STRING_TYPE);
    } else {
        eaf_compile_error(expr->line_number,
...
eval_compare_expression_boolean(Expression *expr,
                                EVM_Boolean left, EVM_Boolean right)
{
    ...
    expr->type = eaf_alloc_type_specifier(EVM_BOOLEAN_TYPE);
...
eval_compare_expression_string(Expression *expr,
                               EVM_Char *left, EVM_Char *right)
{
    int cmp;

    cmp = evm_wcscmp(left, right);
...
fix_logical_and_or_expression(Block *current_block, Expression *expr)
{
    ...
    if (eaf_is_boolean(expr->u.binary_expression.left->type)
        && eaf_is_boolean(expr->u.binary_expression.right->type)) {
        expr->type = eaf_alloc_type_specifier(EVM_BOOLEAN_TYPE);
    } else {
        eaf_compile_error(expr->line_number,
...
fix_minus_expression(Block *current_block, Expression *expr)
{
    ...
    if (!eaf_is_int(expr->u.minus_expression->type)
        && !eaf_is_double(expr->u.minus_expression->type)) {
        eaf_compile_error(expr->line_number,
...
fix_logical_not_expression(Block *current_block, Expression *expr)
{
    ...
    if (!eaf_is_boolean(expr->u.logical_not->type)) {
        eaf_compile_error(expr->line_number,
...
fix_function_call_expression(Block *current_block, Expression *expr)
{
    ...
    fd = eaf_search_function(func_expr->u.identifier.name);
    if (fd == NULL) {
        eaf_compile_error(expr->line_number,
    ...
    expr->type = eaf_alloc_type_specifier(fd->type->basic_type);
...
fix_inc_dec_expression(Block *current_block, Expression *expr)
{
    ...
    if (!eaf_is_int(expr->u.inc_dec.operand->type)) {
        eaf_compile_error(expr->line_number,
...
fix_expression(Block *current_block, Expression *expr)
{
    ...
    case BOOLEAN_EXPRESSION:
        expr->type = eaf_alloc_type_specifier(DVM_BOOLEAN_TYPE);
        break;
    case INT_EXPRESSION:
        expr->type = eaf_alloc_type_specifier(DVM_INT_TYPE);
        break;
    case DOUBLE_EXPRESSION:
        expr->type = eaf_alloc_type_specifier(DVM_DOUBLE_TYPE);
        break;
    case STRING_EXPRESSION:
        expr->type = eaf_alloc_type_specifier(DVM_STRING_TYPE);
        break;
                     ...
                     FunctionDefinition *fd)
{
    ...
    if (return_value == NULL) {
        ...
        case EVM_BOOLEAN_TYPE:
            return_value = eaf_alloc_expression(BOOLEAN_EXPRESSION);
            return_value->u.boolean_value = EVM_FALSE;
            break;
        case EVM_INT_TYPE:
            return_value = eaf_alloc_expression(INT_EXPRESSION);
            return_value->u.int_value = 0;
            break;
        case EVM_DOUBLE_TYPE:
            return_value = eaf_alloc_expression(DOUBLE_EXPRESSION);
            return_value->u.int_value = 0.0;
            break;
        case EVM_STRING_TYPE:
            return_value = eaf_alloc_expression(STRING_EXPRESSION);
...
{
    if (eaf_search_declaration(decl->name, current_block)) {
        eaf_compile_error(line_number,
    ...
    if (current_block) {
        current_block->declaration_list
            = eaf_chain_declaration(current_block->declaration_list, decl);
        add_local_variable(fd, decl);
        decl->is_local = EVM_TRUE;
    } else {
        DKC_Compiler *compiler = eaf_get_current_compiler();
        compiler->declaration_list
            = eaf_chain_declaration(compiler->declaration_list, decl);
        decl->is_local = EVM_FALSE;
    }
}
static void
...
add_parameter_as_declaration(FunctionDefinition *fd)
{
    ...
    for (param = fd->parameter; param; param = param->next) {
        if (eaf_search_declaration(param->name, fd->block)) {
            eaf_compile_error(param->line_number,
                              PARAMETER_MULTIPLE_DEFINE_ERR,
                              STRING_MESSAGE_ARGUMENT, "name", param->name,
                              MESSAGE_ARGUMENT_END);
        }
        decl = eaf_alloc_declaration(param->type, param->name);
...
add_return_function(FunctionDefinition *fd)
{
    ...
    ret = eaf_create_return_statement(NULL);
    fix_return_statement(fd->block, &ret->u.return_s, fd);
    *last_p = eaf_create_statement_list(ret);
}

void
eaf_fix_tree(EAF_Compiler *compiler)
{
