...
DBG_Controller 
DBG_create_controller_func(void) //debugコントローラー関数の生成
{
...
void 
DBG_set_debug_level(DBG_Controller controller,int level){　//debugレベルのセット
    controller->current_debug_level=level;
}static void
initialize_debug_write_fp(void) //ファイル書き取りの初期化
{
...
static void
assert_func(FILE *fp, char *file, int line, char *expression,　//例外を吐いた時の関数
            char *fmt,  va_list ap)
{
...
{
    fprintf(fp, "IOerror file..%s line..%d\n", file, line);
    if (fmt) {
        vfprintf(fp, fmt, ap);
    }
}
